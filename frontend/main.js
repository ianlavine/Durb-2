(() => {
  ...
  const KING_ATTACK_HEALTH_EPSILON = 0.05;
  ...
  function normalizeBrassSetting(value) {
    if (typeof value !== 'string') {
      return typeof DEFAULT_MODE_SETTINGS.brass === 'string'
        ? DEFAULT_MODE_SETTINGS.brass
        : 'cross';
    }
    const normalized = value.trim().toLowerCase();
  ...
  function deriveModeFromSettings(settings = selectedSettings) {
    ...
    if (screen === 'semi') {
      return useIntentionalBrass ? 'i-semi' : 'semi';
    }
    return useIntentionalBrass ? 'i-flat' : 'flat';
  }
  ...
  function applySelectedSettings(overrides = {}) {
    ...
    if (Object.prototype.hasOwnProperty.call(overrides, 'brass')) {
      next.brass = normalizeBrassSetting(overrides.brass);
    }
...
function clearBridgeSelection() {
  ...
  function trackKingUnderAttack(nodeId, prevOwnerId, prevHealth, nextOwnerId, nextHealth) {
    if (kingMoveSuppressedNodes.has(nodeId)) {
      kingMoveSuppressedNodes.delete(nodeId);
      if (kingAttackAlertActive && kingAttackNodeId === nodeId && (!Number.isFinite(nextOwnerId) || nextOwnerId !== myPlayerId)) {
        clearKingAttackAlert();
      }
      return;
    }
    ...
    const nodeWasMyKing = Number.isFinite(prevOwnerId) && prevOwnerId === myPlayerId;
    if (!nodeWasMyKing) {
      return;
    }
    ...
    const healthDropped = Number.isFinite(prev) && Number.isFinite(curr) && curr < prev - KING_ATTACK_HEALTH_EPSILON;
    if (healthDropped) {
  ...
  function handleSecondaryPointer(ev) {
    ...
    if (nodeId != null) {
      ...
      if (node && activeAbility !== 'reverse') {
        const useBrass = isCrossLikeModeActive() && !areBrassPipesDisabled();
