  ...
  function applyNodeMovements(movements) {
    ...
    movements.forEach((entry) => {
      ...
      if (Array.isArray(entry)) {
        ...
        x = entry.x;
        y = entry.y;
        if (Array.isArray(entry.limitReasons)) {
          limitReasons = entry.limitReasons.map((reason) => String(reason));
        } else if (typeof entry.limitReason === 'string') {
          limitReasons = [entry.limitReason];
        }
        if (entry.limited) {
          movementLimited = true;
        }
        if (Object.prototype.hasOwnProperty.call(entry, 'moved')) {
          movedFlag = Boolean(entry.moved);
        }
      } else {
        return;
      }
      ...
      if (movementLimited && limitReasons.includes('collision-limit')) {
        console.warn(
          `[sharp-angle] Node ${id} movement limited by existing pipes (${movedFlag ? 'partial' : 'blocked'})`
        );
      }
