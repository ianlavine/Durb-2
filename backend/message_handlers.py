    ...
    async def handle_build_bridge(
        ...
        if node_movements:
            for movement in node_movements:
                ...
                try:
                    ...
                    continue
                payload: Dict[str, Any] = {"nodeId": node_int, "x": x_val, "y": y_val}
                if "moved" in movement:
                    payload["moved"] = bool(movement.get("moved", True))
                if movement.get("limited"):
                    payload["limited"] = True
                limit_reasons = movement.get("limitReasons") or movement.get("limitReason")
                if isinstance(limit_reasons, (list, tuple)):
                    payload["limitReasons"] = [str(reason) for reason in limit_reasons if reason]
                elif isinstance(limit_reasons, str):
                    payload["limitReasons"] = [limit_reasons]
                movement_payloads.append(payload)
        ...
        if new_edge:
            ...
            if movement_payloads:
                edge_update_message["nodeMovements"] = movement_payloads
            if removed_edges:
